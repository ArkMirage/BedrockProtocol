// <auto-generated>
//     由 Protocol.Gen 自动生成, 请勿手动修改
// </auto-generated>
using System;
using System.Buffers.Binary;
using System.Collections.Generic;
using System.IO;
using System.Numerics;
using OneOf;
using Protocol.Utility;
using Protocol.Utility.IO;

namespace Protocol.Packets
{
	public class ChangeMobPropertyPacket : IPacket
	{
		public override int PacketId => 182;

		public Protocol.Types.ActorUniqueID ActorId { get; set; }
		public string PropertyName { get; set; }
		public bool BoolComponentValue { get; set; }
		public string StringComponentValue { get; set; }
		public int IntComponentValue { get; set; } 
		public float FloatComponentValue { get; set; }

		public override void Read(MemoryStreamReader reader)
		{
			base.Read(reader);
			ActorId = new Protocol.Types.ActorUniqueID();
			ActorId.Read(reader);
			PropertyName = reader.ReadLengthPrefixedString();
			BoolComponentValue = reader.ReadByte() != 0;
			StringComponentValue = reader.ReadLengthPrefixedString();
			IntComponentValue = VarInt.ReadInt32(reader); 
			FloatComponentValue = reader.ReadSingle();
		}

		public override void Write(MemoryStreamWriter writer)
		{
			base.Write(writer);
			ActorId.Write(writer);
			writer.WriteLengthPrefixedString(PropertyName);
			writer.WriteByte(BoolComponentValue ? (byte)1 : (byte)0);
			writer.WriteLengthPrefixedString(StringComponentValue);
			writer.WriteVarInt32(IntComponentValue); 
			writer.WriteSingle(FloatComponentValue);
		}
	}
}
